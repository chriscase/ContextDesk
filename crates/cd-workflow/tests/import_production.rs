...
#[test]
fn reviewed_bindings_apply_per_source_not_all_or_nothing() {
    let cache = tempfile::tempdir().unwrap();
    let source = tempfile::tempdir().unwrap();
    write_file(
        &source.path().join("app.log"),
        b"2024-05-03 12:24:22,729 INFO - reviewed source\n",
    );
    write_file(&source.path().join("other.log"), LOG_B);

    let store =
        ReviewedFormatStore::open(cache.path().join("reviewed_formats")).expect("open store");
    store
        .save(&ReviewedFormat {
            schema_id: REVIEWED_FORMAT_SCHEMA_ID.into(),
            min_reader_version: 1,
            format_id: "test.app-log".into(),
            version: 1,
            name: "Reviewed app log".into(),
    ...
    assert_eq!(
        ...
        1,
        "exactly the matching source gets a binding, not both: {outcome:?}"
    );
    assert!(outcome.reviewed_formats_applied[0]
        .source_identity
        .ends_with("app.log"));
    assert_eq!(outcome.sources_selected, 2, "both sources still import");
}

/// A mix of one importable log, one hidden (ignored) file, and one binary
/// (unsupported) file in one import — the summary's buckets must sum
/// exactly to what was examined, `partial` must be true (not everything
/// discovered became events), and the corpus itself still publishes with
/// only the real log's events.
#[test]
...
/// `crates/cd-core/tests/reviewed_format_wiring.rs`'s own
/// `ingest_direct_dir_zip_nested_with_bindings_cancel_and_retry` pattern,
/// at the `cd_workflow` boundary instead of calling `cd_core::ingest`
/// directly.
#[test]
