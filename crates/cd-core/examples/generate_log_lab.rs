...
use log_lab_generator::{
    generate_behavior, generate_compact, generate_scale, is_behavior_profile,
    write_performance_template, BehaviorControls, DEFAULT_LARGE_EVENT_COUNT, LARGE_PROFILE,
    MEDIUM_EVENT_COUNT, MEDIUM_PROFILE, PAGING_STRESS_PROFILE, SEVEN_DAY_PROFILE, SMALL_PROFILE,
    UI_MEDIUM_PROFILE,
};
...
fn run() -> log_lab_generator::LabResult<()> {
    ...
    let summary = match profile.as_str() {
        ...
        _ => {
            return Err(format!(
            "unsupported profile: {profile} (small|medium|large|ui-medium|seven-day|paging-stress)"
        )
            .into())
        }
    };
