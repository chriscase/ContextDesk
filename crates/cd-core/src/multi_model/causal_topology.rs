...
pub enum CausalTopologyDeriveError {
    ...
    UnsafeIdentity,
    /// The same evidence id would belong to two distinct identities.
    DuplicateId,
}
...
pub fn derive_host_causal_topology(
    ...
    let class_by_id = classifications
        .iter()
        .map(|class| (class.evidence_id.as_str(), class))
    ...
    for finding in findings {
        for claim in &finding.claims {
            if let Some(assignment) = assign_claim(
                ...
                assignments
                    .entry((claim.candidate_id.clone(), claim.claim_id.clone()))
                    .and_modify(|existing| merge_assignment(existing, &assignment))
...
fn merge_assignment(existing: &mut ClaimAssignment, incoming: &ClaimAssignment) {
    if incoming.decoy {
        existing.decoy = true;
        existing.kind = CausalRelationKind::UnrelatedCompeting;
    } else if existing.decoy {
        // Independent/noise already won.
    } else if incoming.kind == CausalRelationKind::Recovery
        || existing.kind == CausalRelationKind::Recovery
    {
        existing.kind = CausalRelationKind::Recovery;
    } else if incoming.kind == CausalRelationKind::UnrelatedCompeting {
        if existing.kind == CausalRelationKind::InitiatingTrigger
            || existing.kind == CausalRelationKind::PropagatedSymptom
        {
            // A competing/unrelated reading does not override an already
            // proven candidate-scope trigger or symptom on the same pair.
        } else {
            existing.kind = CausalRelationKind::UnrelatedCompeting;
        }
    }
...
fn class_of<'a>(
    class_by_id: &BTreeMap<&str, &'a HostEvidenceClassV1>,
    evidence_id: &str,
) -> HostEvidenceClassV1 {
    class_by_id
        .get(evidence_id)
