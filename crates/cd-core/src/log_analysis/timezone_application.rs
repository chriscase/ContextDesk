...
pub fn preview_source_timezone(
    ...
    // Preview never persists, so the basis carried here is never stored; any
    // valid variant recomputes an identical preview.
...
///
/// Always records [`TimezoneDeclarationBasis::UserDeclared`] — the reviewed,
/// previewed, human-confirmed path. A caller applying a configured default
/// with no in-the-moment review (an import's zero-touch aggregation, for
/// instance) must use [`apply_source_timezones_with_basis`] instead, so the
/// durable declaration honestly records which one happened.
