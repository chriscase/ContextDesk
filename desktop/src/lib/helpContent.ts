...
export const HELP_TIMELINE_NAVIGATOR: HelpTipContent = {
  ...
  currentState:
    "The timeline is visible by default on desktop and has a broad chart scrubber. Moving previews synchronized compact values on the log and metric tracks; releasing performs one seek. Right-click a log bucket, or focus it and press Shift+F10, for the richer exact breakdown. A validated metric JSON document can be attached to the exact corpus and restored after Explorer or application restart. It remains outside agent context. Tracks render only when the corpus and metric series share reliable wall-clock time and overlap; otherwise the attachment remains stored with a visible reason. Metric tracks show bounded spike-preserving summaries; brush a range and choose Zoom to selection for finer samples and a matching bounded log histogram. That control becomes Reset full range while zoomed. Collapse stops subsequent timeline and metric work until it is reopened.",
  useWhen:
