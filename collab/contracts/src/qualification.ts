...
export const QUALIFICATION_BACKENDS = ["memory", "postgres"] as const;
...
export interface QualificationConcurrencyV1 {
...
export interface QualificationReportV1 {
  ...
  concurrency: QualificationConcurrencyV1;
...
const concurrencyShape: ObjectShape = {
  ...
  observedMaxActive: f.req(f.u64),
  sameSnapshot: f.req(f.bool),
  durableRunCount: f.req(f.u64),
};
...
export function renderQualificationSummary(report: QualificationReportV1): string {
  const skipped = report.liveProfiles
    .filter((profile) => !profile.ran)
    .map((profile) => `${profile.alias} (${profile.skippedReason ?? "skipped"})`);
  return [
    `qualification backend=${report.backend}`,
    `steps=${report.steps.filter((step) => step.status === "passed").length}/${report.steps.length} passed`,
    `lanes durable=${report.concurrency.durableRunCount} max_active=${report.concurrency.observedMaxActive}/${report.concurrency.configuredLimit} same_snapshot=${report.concurrency.sameSnapshot}`,
    `comparison shared=${report.comparison.sharedEvidenceCount} unique=${report.comparison.uniqueEvidenceCount} disagreements=${report.comparison.disagreementCount} unknowns=${report.comparison.unknownCount} questions=${report.comparison.questionPathCount}`,
    `decision accepted=${report.comparison.acceptedDecision} gold=${report.comparison.goldAligned} lineage ${report.lineage.predecessorPackageAlias} -> ${report.lineage.successorPackageAlias}`,
    `live skipped: ${skipped.join(", ") || "none"}`,
  ].join("\n");
}
