...
describe.skipIf(!adminUrl())("migrations", () => {
  it("applies and rolls back all migrations in order", async () => {
    await withDisposableDb(async (client) => {
      ...
      expect(up.applied).toContain("015_contribution_write_intents");
      ...
      expect((await migrateDown(client)).rolledBack).toBe("015_contribution_write_intents");
  ...
  it("dry-run lists pending SQL without applying", async () => {
    await withDisposableDb(async (client) => {
      ...
      expect(dry.pending).toContain("015_contribution_write_intents");
