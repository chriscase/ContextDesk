...
function readableDomain(artifacts: { id: string; privacyClass: string }[]) {
  return {
    async getReadableHeldArtifact(
      ...
      if (artifact?.privacyClass === "owner_only" && !canReadPrivate) return null;
      return artifact;
    },
  } as unknown as Pick<CaseService, "getCase" | "getReadableHeldArtifact">;
}
...
function corpusPort(fileCount: number) {
  ...
  return {
    port: createLogTimeCasePort({
      ...
      domain: readableDomain(artifacts),
...
describe("log corpus builder capacity", () => {
  it("keeps accepted rotated logs in stable order and excludes arbitrary suffixes", async () => {
    const storage = evidenceFor();
    const artifacts = [
      { id: "b", relativePath: "logs/service.log.1", contentHash: X_HASH, byteLength: 1, privacyClass: "share_safe" },
      { id: "a", relativePath: "logs/service.log-2026-08-25", contentHash: X_HASH, byteLength: 1, privacyClass: "share_safe" },
      { id: "c", relativePath: "logs/service.log.exe", contentHash: X_HASH, byteLength: 1, privacyClass: "share_safe" },
    ];
    ...
    await expect(port.listCorpusFilesForCase("case-1", ACTOR, false, false)).resolves.toEqual([
  ...
  it("keeps the first committed duplicate path and skips missing stored bytes", async () => {
    ...
    const artifacts = [
      { id: "first", relativePath: "logs/a.log", contentHash: X_HASH, byteLength: 1, privacyClass: "share_safe" },
      { id: "second", relativePath: "logs/a.log", contentHash: Y_HASH, byteLength: 1, privacyClass: "share_safe" },
      { id: "present", relativePath: "logs/b.log", contentHash: Y_HASH, byteLength: 1, privacyClass: "share_safe" },
    ];
    const port = createLogTimeCasePort({
      cases: { listArtifactsByCase: async () => artifacts } as unknown as CaseStore,
      domain: readableDomain(artifacts),
  ...
  it("returns all 4,096 eligible files and rejects file 4,097 before storage", async () => {
    ...
    await expect(accepted.port.listCorpusFilesForCase("case-1", ACTOR, false, false))
      .resolves.toHaveLength(CORPUS_INTAKE_LIMITS.maxFileCount);
    const rejected = corpusPort(CORPUS_INTAKE_LIMITS.maxFileCount + 1);
    await expect(rejected.port.listCorpusFilesForCase("case-1", ACTOR, false, false))
  ...
  it("rejects item and aggregate catalog overflow before probing even missing storage", async () => {
    for (const lengths of [
      ...
      await expect(
        port.listCorpusFilesForCase("case-1", ACTOR, false, false),
      ).rejects.toThrow(LogTimeRequestError);
      expect(storage.calls).toEqual({ get: 0, head: 0, openRead: 0 });
    }
  });

  it("denies a mixed private corpus before storage and keeps share-safe reads allowed", async () => {
    const storage = evidenceFor();
    const artifacts = [
      {
        id: "public",
    ...
    await expect(
      port.listCorpusFilesForCase("case-1", ACTOR, false, true),
    ).resolves.toHaveLength(2);
    expect(storage.calls).toEqual({ get: 0, head: 2, openRead: 2 });
  });
});
