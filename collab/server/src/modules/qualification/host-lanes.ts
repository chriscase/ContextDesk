/**
 * Hermetic fake host lanes for qualification.
 *
 * Collab does not own the Rust live-bridge runner. These fakes stand in for
 * ContextDesk-owned lanes at the Experiment Lab import seam: same snapshot
 * identity, bounded overlap, cancellation, deadlines, and partial results.
 * They never persist a second job/lease table.
 */
    ...
    results: input.lanes.map((lane, index) => {
      return (
        ...
        finished(
          lane,
          input.snapshotFingerprint,
          stopped === "deadline" ? "timeout" : "cancelled",
          [],
        )
      );
    }),
...
export async function exerciseHostLaneContracts(snapshotFingerprint: string): Promise<{
  ...
  const bounded = await runFakeHostLanes({
    snapshotFingerprint,
    lanes,
    scripts,
  ...
  if (bounded.maxActive > DEFAULT_HOST_LANE_CONCURRENCY) {
    throw new Error("host lanes exceeded the configured concurrency bound");
  }
  if (bounded.maxActive < 2) {
    throw new Error("host lanes did not overlap under the concurrency bound");
  }
  ...
  if (durableRunCount !== lanes.length) {
    throw new Error("host lanes did not mint a durable run id per candidate");
  }
  ...
  if (!cancelled.results.some((row) => row.runStatus === "cancelled")) {
    throw new Error("host lanes did not surface cancellation");
  }
  ...
  if (!deadline.results.some((row) => row.runStatus === "timeout") || deadline.stopped !== "deadline") {
    throw new Error("host lanes did not surface a deadline timeout");
  }
  ...
  if (partial.stopped !== "lane_failed") {
    throw new Error("host lanes did not stop after a failed lane");
  }
  if (!partial.results.some((row) => row.runStatus === "failed")) {
    throw new Error("host lanes did not record a failed lane");
  }
  if (
    !partial.results.some(
      (row) => row.runStatus === "cancelled" || row.runStatus === "completed" || row.runStatus === "timeout",
    )
  ) {
    throw new Error("host lanes did not keep partial results after a failed lane");
  }

  return {
    configuredLimit: DEFAULT_HOST_LANE_CONCURRENCY,
    observedMaxActive: bounded.maxActive,
    sameSnapshot,
    durableRunCount,
    cancelled: true,
    deadline: true,
