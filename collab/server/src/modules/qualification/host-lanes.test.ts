...
describe("fake host lanes", () => {
  it("keeps overlap at the configured bound and mints durable same-snapshot ids", async () => {
    const run = await runFakeHostLanes({
      ...
      lanes,
      scripts: { "cand-a": { delayMs: 40 }, "cand-b": { delayMs: 40 }, "cand-c": { delayMs: 40 } },
      maxConcurrency: 2,
      deadlineMs: 5_000,
    });
    ...
    expect(run.results.every((row) => row.snapshotFingerprint === SNAP && row.runStatus === "completed")).toBe(true);
  });
