...
# Optional provider-location identity pin for either filesystem or S3. Leave
# this commented to preserve legacy unbound behavior. Generate and retain one
# canonical lowercase UUID per filesystem root or S3 bucket plus prefix; do not
# copy this placeholder as an identity and do not rotate it with credentials.
# Back up the configured UUID; there is no automatic rotation, deletion, repair,
# or migration workflow for the private provider marker.
# COLLAB_EVIDENCE_PROVIDER_INSTANCE_ID=replace-with-one-canonical-lowercase-uuid
