...
class CountingCaseService extends CaseService {
  listCasesCalls = 0;
  getCaseCalls = 0;
  overviewVisibilityBoundaryCalls = 0;
  ...
  override async getCase(id: string, actor: Actor, isAdmin: boolean) {
    this.getCaseCalls += 1;
    return super.getCase(id, actor, isAdmin);
  }

  override async overviewVisibilityBoundary(
    scope: Parameters<CaseService["overviewVisibilityBoundary"]>[0],
  ) {
    this.overviewVisibilityBoundaryCalls += 1;
    return super.overviewVisibilityBoundary(scope);
  }
}
...
describe("GET /api/overview", () => {
  ...
  it("enforces caps without per-case store fan-out or listCases inventory", async () => {
    await withApp(async ({ app, jobs, experiments, domain }) => {
      ...
      const beforeCaseReads = domain.getCaseCalls;
      const beforeVisibilityBoundaries = domain.overviewVisibilityBoundaryCalls;
      const { body } = await overviewOf(app, alice);
      ...
      expect(domain.listCasesCalls).toBe(0);
      expect(domain.getCaseCalls).toBe(beforeCaseReads);
      expect(domain.overviewVisibilityBoundaryCalls).toBe(beforeVisibilityBoundaries + 1);
    });
  });
});
