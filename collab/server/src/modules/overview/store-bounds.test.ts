...
describe("overview store helpers stay bounded in memory", () => {
  it("caps jobs, activity, presence, and proposed decisions without a hidden case", async () => {
    ...
    const visibility = await cases.overviewVisibilityBoundary(aliceScope);
    expect(visibility?.caseTitle(CASE_A)).toBe("Visible");
    expect(visibility?.caseTitle(CASE_B)).toBeNull();
