...
function EvidenceCard() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (privacyClass === "owner_only" && !runtime.capabilities.canReadPrivate) {
      setPrivacyClass("");
      if (fileRef.current) fileRef.current.value = "";
      setFeedback({ tone: "danger", text: "Private evidence access changed. Choose a privacy level again before attaching this file." });
      return;
  ...
  return (
    <StrategyPanel title="Supporting material" titleId="beacon-evidence-title" description="Attach logs, emails, and files to the shared evidence inventory." busy={busy}>
      ...
      {runtime.capabilities.canUpload ? <form className="beacon__upload" onSubmit={(event) => void submit(event)}><label className="beacon__field"><span>File (up to {(MAX_EVIDENCE_UPLOAD_BYTES / 1_000_000).toFixed(0)} MB)</span><input ref={fileRef} type="file" required /></label><label className="beacon__field"><span>Kind</span><select value={kind} onChange={(event) => setKind(event.target.value as ArtifactKind)}><option value="attachment">Attachment</option><option value="log">Log</option><option value="email">Email</option></select></label><label className="beacon__field"><span>Privacy</span><select value={privacyClass} required onChange={(event) => setPrivacyClass(event.target.value === "owner_only" && runtime.capabilities.canReadPrivate ? "owner_only" : event.target.value === "share_safe" ? "share_safe" : "")}><option value="">Choose privacy</option><option value="share_safe">Share safe</option>{runtime.capabilities.canReadPrivate ? <option value="owner_only">Owner only</option> : null}</select></label><label className="beacon__field beacon__field--wide"><span>Why does this matter?</span><input value={summary} onChange={(event) => setSummary(event.target.value)} required /></label>{feedback ? <StrategyStateNotice tone={feedback.tone} role={feedback.tone === "danger" ? "alert" : "status"}>{feedback.text}</StrategyStateNotice> : null}<button type="submit" disabled={busy || !privacyClass || !runtime.commands.uploadEvidence}>{busy ? "Attaching…" : runtime.commands.uploadEvidence ? "Attach evidence" : "Preparing upload…"}</button></form> : <StrategyStateNotice title="Evidence upload is read-only">You can review supporting material, but your current access cannot attach a file.</StrategyStateNotice>}
    </StrategyPanel>
  );
}
