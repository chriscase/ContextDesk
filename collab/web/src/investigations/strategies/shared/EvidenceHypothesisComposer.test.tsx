...
const EVIDENCE: readonly EvidenceHypothesisEvidence[] = [
...
function mount(overrides: Partial<EvidenceHypothesisComposerProps> = {}) {
  let props: EvidenceHypothesisComposerProps = {
  ...
  const view = render(<EvidenceHypothesisComposer {...props} />);
  return {
    ...view,
    rerenderComposer(next: Partial<EvidenceHypothesisComposerProps>) {
      props = { ...props, ...next };
      view.rerender(<EvidenceHypothesisComposer {...props} />);
    },
  };
}
...
describe("Keystone K2 evidence-linked hypothesis composer", () => {
  ...
  it("sends exactly one field-bounded hypothesis and clears only after authoritative success", async () => {
    ...
    expect(command).toHaveBeenCalledWith(expect.objectContaining({
      ...
      idempotencyKey: expect.stringMatching(/^evidence-hypothesis-/),
    }));
  ...
  it("reuses one idempotency key after an ambiguous response loss and rotates after success", async () => {
    ...
    expect(firstKey).toMatch(/^evidence-hypothesis-/);
    ...
    expect(postSuccessKey).toMatch(/^evidence-hypothesis-/);
  ...
  it("rotates an ambiguous retry key after the user explicitly clears and restores the draft", async () => {
    ...
    expect(restoredKey).toMatch(/^evidence-hypothesis-/);
