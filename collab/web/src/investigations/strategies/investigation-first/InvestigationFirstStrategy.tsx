...
export function InvestigationFirstStrategy(props: InvestigationStrategyShellProps) {
  ...
  function renderList() {
    ...
    return <section className="investigation-first__browse" aria-labelledby="investigation-first-browse-title" aria-busy={busy}>
      ...
      <div className="investigation-first__filters"><label><span>Search</span><input className="login__input" type="search" value={query} onChange={(event) => updateCollectionQuery({ q: event.target.value })} placeholder="Title, product, build, or problem" aria-label="Search investigations" /></label><label><span>Status</span><select className="login__input" value={statusFilter} onChange={(event) => updateCollectionQuery({ status: event.target.value })} aria-label="Filter investigations by status"><option value="all">All statuses</option><option value="open">Open</option><option value="monitoring">Monitoring</option><option value="resolved">Resolved</option><option value="archived">Archived</option></select></label>{collection.enabled && props.collectionQuery ? <CollectionDiscoveryFilters query={props.collectionQuery} onQueryChange={props.onCollectionQueryChange} facets={collectionView.availability === "available" ? collectionView.value.facets : null} showEntity cursorRestartNotice={runtime.resources.investigationCollectionNotice} /> : null}<label className="investigation-first__checkbox"><input type="checkbox" checked={includeArchived} onChange={(event) => updateCollectionQuery({ includeArchived: event.target.checked })} /> <span>Include archived</span></label></div>
      ...
      {listView.availability === "available" && filteredCases.length === 0 ? <p className="investigation-first__empty">{collectionEmptyMessage(props.collectionQuery ? shareableQueryNarrows(props.collectionQuery) : query.trim().length > 0 || statusFilter !== "all", { canCreate: runtime.capabilities.canCreate })}</p> : null}
  ...
  function renderDetail() {
    ...
    return <section className="investigation-first__detail" aria-labelledby="investigation-first-detail-title" aria-busy={investigation.refresh === "loading"}>
      ...
      <RuntimeCoordinationControl investigation={selected} />
