...
export function InvestigationFirstStrategy(props: InvestigationStrategyShellProps) {
  ...
  const hasAdvancedFacetFilter = Boolean(props.collectionQuery?.entityId || props.collectionQuery?.contributorId);
  ...
  function renderList() {
    ...
    return <section className="investigation-first__browse" aria-labelledby="investigation-first-browse-title" aria-busy={busy}>
      ...
      {listView.availability === "available" && filteredCases.length === 0 ? <p className="investigation-first__empty">{hasAdvancedFacetFilter ? "No investigations match the selected recorded values. Try a different filter or clear advanced filters." : runtime.capabilities.canCreate ? "No investigations match this view. Try a different search or create a new one." : "No investigations match this view. Try a different search."}</p> : null}
