...
export function useUiStrategyGovernance(input: {
  ...
  useEffect(() => {
    ...
    const notice = reconciliationNoticeRef.current;
    const matchingNotice = notice?.identityId === input.identityId
      && notice.authorityGeneration === input.authorityGeneration ? notice.text : "";
    if (!matchingNotice) reconciliationNoticeRef.current = null;
    setEffective(SAFE_EFFECTIVE);
    setStatus("loading");
    setMessage(matchingNotice || "Loading the workspace investigation-experience policy…");
    void protectedApiFetch("/api/ui-strategies/effective", { signal: controller.signal })
      ...
      .then((loaded) => {
        ...
        setEffective(loaded);
        setStatus("ready");
        const reconciliation = reconciliationNoticeRef.current;
        const reconciled = reconciliation?.identityId === input.identityId
          && reconciliation.authorityGeneration === input.authorityGeneration
          ? reconciliation.text : "";
        reconciliationNoticeRef.current = null;
  ...
  const savePreference = useCallback(async (strategyId: UiStrategyId): Promise<boolean> => {
    ...
    try {
      ...
      if (response.status === 409) {
        setEffective(SAFE_EFFECTIVE);
        setStatus("conflict");
        const text = "Your preference was not saved because the workspace policy or your saved choice changed.";
        reconciliationNoticeRef.current = { identityId: start.identityId, authorityGeneration: start.authorityGeneration, text };
        setMessage(`${text} War Room is active while current authority is reloaded.`);
      ...
      setEffective(SAFE_EFFECTIVE);
      setStatus("unavailable");
      const text = "Your preference was not confirmed.";
      reconciliationNoticeRef.current = { identityId: start.identityId, authorityGeneration: start.authorityGeneration, text };
      setMessage(`${text} War Room is active while current authority is reloaded.`);
      setGeneration((value) => value + 1);
      return false;
