  ...
  function renderDetail() {
    ...
    return (
      <section className="investigation-first__detail" aria-labelledby="investigation-first-detail-title">
        ...
        {selected.status === "archived" || props.canLead ? <LifecyclePanel caseId={selected.id} status={selected.status} canLead={props.canLead && !props.readOnly} readOnly={props.readOnly} onChanged={async () => { await refreshCases(); await refreshDetail(selected.id); }} /> : null}
      </section>
    );
  }
