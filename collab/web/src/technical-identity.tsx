...
export function TechnicalIdentifiers(props: {
  ...
  summary?: string;
  /**
   * Names the record these identifiers belong to, in words.
   *
   * Two records on one screen otherwise give their copy controls the same
   * accessible name — and a surface that already labels a control "Copy
   * snapshot fingerprint" would collide with this one outright. Screen-reader
   * users and tests both need the pair to be tellable apart, so the record
   * name is folded into every control and announcement here.
   */
  record?: string;
  className?: string;
}) {
  ...
  return (
    <details className={props.className ? `technical-id ${props.className}` : "technical-id"}>
          ...
          <div className="technical-id__row" key={`${item.label}:${item.value}`}>
            ...
            <dd>
              ...
              <button
                ...
                aria-label={`Copy ${item.label.toLowerCase()}${of}`}
      ...
      {/* Only present while it has something to say. An always-rendered live
          region puts an empty status node in the accessibility tree for every
          record on screen, which is the noise this component set out to avoid. */}
      {message ? (
        <p className="technical-id__status" role="status" aria-label={`Identifier copy result${of}`}>
          {message}
        </p>
      ) : null}
    </details>
