  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        recorded:
          "Each accepted file records relative path, media type, digest, byte length, privacy class, uploader identity, import time, source attribution, and the intake batch id. Duplicate digests reuse the existing artifact and add a new path attribution on the timeline. Deep links exist for the batch on Capture and each evidence item on Analyze.",
        limits:
          "Archives are capped at 8 MiB, expanded bytes at 12 MiB, 64 files, path depth 8, path length 240, 1 MB per file, compression ratio 20, and 5 seconds of processing. ZIP extraction rejects absolute paths, traversal, drive/UNC paths, symlinks/hardlinks, device entries, duplicate normalized paths, nested archives, encrypted archives, ZIP64, and malformed central-directory metadata. Allowlisted extensions are .log, .txt, .json, .csv, .xml, .eml, and .md. Binary or unknown types are rejected or metadata-only — they are never silently decoded as text. share_safe runs the privacy scan before content becomes triage-visible; original bytes stay owner_only unless that class is chosen. Marking share_safe does not scrub the file. The global source catalog is not the intake path for these uploads.",
        actions: [{ label: "Open the Capture stage", go: { stage: "capture" } }],
      },
    ],
  },
