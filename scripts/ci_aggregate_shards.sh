...
echo
printf '%-8s %-8s %-14s %-9s %-9s %-9s %s\n' shard result units passed failed ignored seconds
printf '%s\n' "----------------------------------------------------------------------------------"
...
while [ "$i" -le "$SHARDS" ]; do
  ...
  printf '%-8s %-8s %-14s %-9s %-9s %-9s %s\n' "$i" "$status" "$ufin/$utot" "$p" "$f" "$ig" "$secs"
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
  {
    ...
    echo
    echo "| shard | result | units | passed | failed | ignored | seconds |"
    echo "| --- | --- | --- | --- | --- | --- | --- |"
    i=1
    while [ "$i" -le "$SHARDS" ]; do
      file=$(awk -F'\t' -v want="$i" '$1 == want { print $2 }' "$tmp/index" | head -n 1)
      if [ -n "$file" ]; then
        jq -r '"| \(.shard) | \(.status) | \(.units_finished)/\(.units_total) | \(.tests_passed) | \(.tests_failed) | \(.tests_ignored) | \(.seconds) |"' "$file"
      else
        echo "| $i | **missing** | - | - | - | - | - |"
      fi
      i=$((i + 1))
