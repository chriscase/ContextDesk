...
| Linked chat            | Compact rail with chat switcher; **New** (or the first Return/Send) creates a durable corpus-linked session only through the host-governed link path before any turn. Concurrent New/Send/Return create races coalesce to **one** durable session. Generic chat save cannot forge that link. Switching chats is race-safe (a slow load cannot overwrite the active chat). Long threads use a bounded virtualized window; history stays persisted. For a conservatively recognized broad question, the host prepares bounded deterministic evidence before the model participates. Focused questions and explicitly requested eligible sources use staged bounded tools. No planning-only prose is accepted as evidence. **Sources** on a linked answer: only lowercase `log_event:<digits>` / `log_template:<digits>` (u64, no leading zeros) are governed evidence identities (never workspace files). Clicks open the citation's host-authored corpus via exact-nav (one-shot event reveal or template open); IDs stay strings through routing and never silently widen past safe integers. Host-authored corpus provenance is kept with each log citation so reopen uses the original corpus after detach/relink; legacy citations without provenance fail closed without substituting the current corpus; malformed schemes stay unavailable |
...
target and its content fingerprint still agree; when the number matches but the
content does not, the rule is labeled **stale — template changed** and hides
nothing. Silently re-pointing a rule at different content would hide evidence
nobody reviewed.

**Unusable rules fail closed.** A rule whose saved predicate is no longer valid,
or which claims the same target as another enabled rule, hides nothing and
