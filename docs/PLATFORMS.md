...
|-----|--------|-------|---------|
| `rust` (fmt, clippy, examples, smoke) | yes (`rust (ubuntu-latest)`; tests compiled with `--no-run` as shard cache warmup) | yes | yes (#178) |
| `rust tests` (`cargo test --workspace`) | yes — eight deterministic shards + fail-closed `rust tests (ubuntu aggregate)` (#874) | yes (monolithic step on `rust (macos-latest)`) | yes (monolithic step on `rust (windows-latest)`) |
...
- **Linux workspace tests** run as eight deterministic shards with a fail-closed aggregate (`docs/DEV.md`, #874). `rust (ubuntu-latest)` no longer executes `cargo test --workspace`; see `docs/CI_REQUIRED_CHECKS.md` before adding a ruleset.
