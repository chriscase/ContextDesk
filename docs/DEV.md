...
| Confluence RO | `confluence_ro` | **Shipped** | PAT in keychain (`confluence/default/pat`); space allowlist; Settings Connectors. **Wire path (#132):** Settings → keychain PAT → `set_confluence` / `apply_host_connectors` → `specs_for_model` exposes `confluence_search`/`confluence_get_page` → dispatch → `cql_search`/`fetch_page`. **Maneuver (#326 PR1):** `confluence_list_children` (page children or space roots), `confluence_get_ancestors`, `confluence_list_attachments` (meta only); `confluence_get_page` supports `format` plain\|meta\|storage\|all; `space_permitted` empty-allowlist semantics; optional `rest_path_mode` / `url_style` on settings. Offline: `cargo test -p cd-core --lib confluence` (wiremock Bearer + children/roots). Harvest/write are later PRs. |
