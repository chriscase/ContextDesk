...
2. Runs the same suite as `CD_SHARD_COUNT` (currently **8**) shards that
   ...
   `cd-core/lib` is executed as eleven complementary filters (`log_analysis::`,
   `runtime`, `services`, `platform`, `control`, `policy_git`, `policy_sources`,
   `policy_stream`, `wire`, `surface`, and a light skip leftover) so the heavy
   library binary is not pinned to one shard. Leftover-heavy slices do not
   share a shard with other `cd-core` units.
   ...
   now platform / control / policy_git / policy_sources / policy_stream / wire
   / surface plus a light skip leftover. Isolated `control` timed out at 57m
   on run `31931230358`; `policy` plus other cd-core tests timed out at 58m
   on run `31936313731`; the policy slice remained in progress until the 60m
   budget on runs `31941510316` and `31949682882`. Leftover-heavy slices are
   split further and may not sit alone or share a shard with other `cd-core`
   units.
