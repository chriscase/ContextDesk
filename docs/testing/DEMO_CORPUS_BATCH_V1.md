...
1. `normalize --output ... --output-format jsonl` (offline preflight)
...
- `report.json` and `report.md` contain case ordinals, bounded status fields,
  ...
  completeness, or interpretation quality.
...
second transport, and separates raw artifacts from the aggregate report. On a
host with PowerShell, the same test also invokes PowerShell's parser. The
