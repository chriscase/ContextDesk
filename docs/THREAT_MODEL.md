...
- The Jira MCP preset accepts only Atlassian's fixed public Rovo MCP endpoint and preflights its DNS result through the remote SSRF policy before launching a separately installed absolute `mcp-remote` command. API tokens are resolved from keychain refs into the child's cleared environment; they never enter connector JSON or argv. Because the proxy performs the actual remote transport, ContextDesk cannot socket-pin its child process after preflight; exact-host enforcement, no arbitrary endpoint, HTTP-only transport, least-privilege Atlassian scopes, host-owned tool classification, and fresh per-action HardWrite confirmation are the compensating controls (#291).
...
| Asset | Trust |
|-------|--------|
| Update payloads | Fetched over HTTPS from GitHub Releases `latest.json` only. GA assemble requires that file's URLs bind to `/releases/download/<exact-tag>/` artifacts (never `/latest/` or another tag) and that signatures bind to those exact bytes |
