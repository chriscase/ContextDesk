...
| Candidate-scoped broad-triage comparison                       | **Experimental (hermetic)** | [bounded multi-stage contract](BOUNDED_MULTI_STAGE_BROAD_TRIAGE.md), shared `cd-core` candidate ledger + final confinement gate | Structural grouping is not a root-cause verdict; parallel-provider acceptance remains open |
...
| Explicit synthetic capability qualification                    | **Partial** | User-triggered probes, cache key isolation, inert tools, Settings control, mock-transport tests (#724), dialect-honest multi-mode ladder (prompted JSON vs native json_object/schema, auto vs forced tools), exact-mode runtime authorization, and DTO/CLI/desktop projection (schema v3; [chat contract](../../testing/OPENAI_CHAT_CONTRACT_V2.md)) | Packaged proof against real tools-enabled/disabled profiles remains owner/environment residual; live native mode support is gateway-dependent |
