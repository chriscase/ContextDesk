...
| Explicit synthetic capability qualification                    | **Partial** | User-triggered probes, cache key isolation, inert tools, Settings control, mock-transport tests (#724), and typed OpenAI chat modes (`response_format` / forced tool) on structured probes (schema v2; [chat contract v2](../../testing/OPENAI_CHAT_CONTRACT_V2.md)) | Packaged proof against real tools-enabled/disabled profiles remains owner/environment residual; live `response_format` support is gateway-dependent |
