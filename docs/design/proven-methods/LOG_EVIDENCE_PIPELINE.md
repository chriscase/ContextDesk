...
| Deterministic logging-quality assessment (fixed finding-code hints) | **Local integration** | [`logging_quality.rs`](../../../crates/cd-core/src/log_analysis/logging_quality.rs) (pure assessor), [`cd-workflow` logging_quality](../../../crates/cd-workflow/src/logging_quality.rs) (atomic publish), CLI `logging-assessment`, Help [`logging-quality-assessment.md`](../../help/log-analysis/logging-quality-assessment.md) | GUI wiring and free-form/LLM “Engineering Improvement Plan” synthesis are explicitly out of scope for this slice |
| Normalized-stream automation gate | **Local integration** | Core streaming validator in [`normalized_log_events.rs`](../../../crates/cd-core/src/normalized_log_events.rs), host-neutral file/directory orchestration in [`cd-workflow`](../../../crates/cd-workflow/src/normalized.rs), and CLI `normalized validate` / `normalized summarize`; `normalize --fail-on-partial` preserves published output while returning a gateable verdict | No public canonicalize command, normalized fast-import path, DuckDB API, or language-specific parser |
| War Room investigation Log workbench (bounded search, chronology, revision-bound freeze) | **Local integration** | Contract `investigation-workbench.ts`, server `collab/server/src/modules/workbench/`, Analyze UI `LogWorkbench.tsx`, host actions `collab-log-time` `search` and `events` in [`collab_log_time.rs`](../../../crates/cd-cli/src/commands/collab_log_time.rs). Search/page/chronology overlay wall-clock UTC from the live host corpus revision (`query_events`); local-ambiguous lines stay without UTC until an explicit timezone apply. Reading is scope-first: the caller's file selection narrows the file list before any evidence is materialized (`listEvidenceDescriptors` then `readEvidenceText` per file), and records are walked in `corpusWindowLines` windows, so no path holds an investigation-sized set of rows. The current local server still fetches and decodes one complete selected evidence file before building those line windows; the window bound is not a byte-streaming or single-file memory bound. Chronology clusters by the requested grouping key; a pin is insert-only and `human_ground_truth` requires a recorded note. Saved views restore filters, time window, sort, grouping, and display, and are keyed by name plus view content so re-saving a changed view records a new one rather than conflicting. Frozen snapshots bind `normalizationRevision`, and both storage backends represent an unobserved revision by omitting the field. A partial read is never presented as a complete answer, and a bounded *page* is distinguished from a truncated *corpus*: a search page spends at most `maxSearchWorkLines` and then returns `nextPageCursor`, an opaque record of the exact corpus position it stopped at, bound to a scope digest and normalization revision so a cursor minted against a moved corpus is refused rather than resumed against different lines. Advancing it can never skip a match, so a zero-match page still carries the search forward and a line past the page budget stays reachable; `coverageComplete` states whether every selected line has now been searched, and `corpusTruncated` is reserved for a scan that stopped and cannot continue (cancelled, or a file whose bytes are unreadable). The legacy `cursor` match ordinal keeps its original meaning for callers written against the first contract. The inventory counts every file's lines and names only files it genuinely could not read; the chronology adds a `corpus` unknown bucket. A time-filtered search is answered by the timestamp authority — the host `search` action, scoped to the selected sources — and is refused outright when that host cannot be reached, rather than answered from intake bytes that carry no offset. Time-range bounds are parsed as full UTC instants and compared as instants; a local time with no zone, an unparsable bound, or an inverted window is refused with actionable copy rather than filtered by string order. The normalized chronology projection is reachable on Analyze as `triage-log-chronology`. | Desktop/CLI Log Explorer chat rail, Drain templates, and package export are not claimed. Host overlay is additive; intake-byte search remains available without the host. Playwright host-backed timezone apply is skipped unless `COLLAB_E2E_LOG_TIME_BIN` is set. PostgreSQL workbench writes take `pg_advisory_xact_lock` on one client `BEGIN…COMMIT` that also binds case timeline via `runWithCaseQueryable`. |
