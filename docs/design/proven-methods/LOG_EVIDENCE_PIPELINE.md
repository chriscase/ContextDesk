...
| War Room investigation Log workbench (bounded search, chronology, revision-bound freeze) | **Local integration** | Contract `investigation-workbench.ts`, server `collab/server/src/modules/workbench/`, Analyze UI `LogWorkbench.tsx`, host action `collab-log-time` `search` in [`collab_log_time.rs`](../../../crates/cd-cli/src/commands/collab_log_time.rs). Search is virtualized and match counts become “at least N” when capped. Time-range filters do not invent a zone for local timestamps. Frozen snapshots may bind `normalizationRevision`. | Desktop/CLI Log Explorer chat rail, Drain templates, and package export are not claimed. Host search is additive; intake-byte search remains available without the host. |
