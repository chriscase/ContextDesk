...
| Role-specific provider/model compatibility qualification | **Local integration** | Shared evidence projection in [`capability_qualification.rs`](../../../crates/cd-core/src/capability_qualification.rs), live adapters in [`capability_qualification.rs`](../../../crates/cd-workflow/src/capability_qualification.rs), and GUI/CLI `Discover → Verify → Choose` surfaces | Current chat suite proves triage contracts only; ordinary chat, attachments, multimodal input, and answer quality require distinct evidence |
