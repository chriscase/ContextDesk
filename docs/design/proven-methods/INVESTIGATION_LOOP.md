...
| War Room activity feed and resource locators | **Local integration** | Collab contract `investigation-activity.ts`, module `collab/server/src/modules/activity/`, Overview consumer in `Cases.tsx`, APIs `GET /api/investigation-activity` and `GET /api/investigation-resources/resolve` | UI filtering/pagination remains residual; locators reauthorize at resolve time and are not tokens. |
