...
| War Room server/contracts integrity | **Local integration** | Snapshot-bound accepted-decision board projection, insert-only contribution write intents, contribution revision CAS, staged ordinary `addEvidence`/`importRun` transactions, honest `sameSnapshot`, and lease-guarded triage job updates in `collab/server` + `collab/contracts` | `file_server_ref` still registers the reference before metadata; fsync durability, `/ready` migration head, empty catalog honesty, and web discussion key reuse remain residual. |
