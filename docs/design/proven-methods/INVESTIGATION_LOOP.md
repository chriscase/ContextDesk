...
| War Room streamed large-corpus intake | **Local integration** | Collab contract `investigation-corpus-stream.ts` (`cd-collab.corpus_intake_preflight.v1`, `cd-collab.corpus_intake_session.v1`, `cd-collab.corpus_intake_session_commit.v1`, `cd-collab.corpus_intake_error.v1`), server `collab/server/src/modules/corpus-intake/` (`spool.ts`, `session.ts`, `session-routes.ts`, `zip-walk.ts`, `classify-stream.ts`), browser driver `collab/web/src/corpus-intake-session.ts`, Capture UI `CorpusIntakePanel.tsx`. A selection above one request's ceiling opens a session, streams bounded binary parts to a disk spool, expands and classifies them a window at a time, and commits one staged member at a time. Preflight states the selected count, the bytes, the configured limit, the validation stages, and the facts it cannot know yet. Every limit is owner-configurable within published ceilings and enforced in the same functions it is advertised from. | Resuming after a browser reload asks for the same selection again, because a page cannot retain file handles; the server-held bytes are reused. Nested archives are refused by default and expand only when an owner raises `maxArchiveDepth`, where a nested member's bytes are charged to the expanded budget both as archive and as members. UTF-16 members are refused unless an owner enables that encoding. |
...
### 6.7 Streamed large-corpus intake

A corpus that will not fit in one request is not a bigger version of the inline
intake; it is a different shape of problem. Base64 inside JSON has a hard
ceiling that has nothing to do with how much evidence an investigation may
hold: a JSON body is parsed from a string, and the runtime cannot build a
string longer than `String::kMaxLength` (2**29 - 24 bytes on 64-bit V8). A
transport limit derived from the corpus allowance rather than from that ceiling
is a promise that cannot be kept — the request is accepted and then fails while
being turned into a string, which reads to the operator as an unexplained
error rather than as a reviewable intake decision.

The streamed lane separates the two:

1. **Preflight.** The client posts a manifest — origin, label, privacy class,
   idempotency key, and one declaration per part. It is checked against the
   resolved limits before a content byte is accepted, and the response states
   the selected count, the bytes, the configured ceiling, the validation stages
   this origin will run, and the facts that are genuinely unknown (an archive's
   expanded size and member list cannot be known from a file picker).
2. **Bounded parts.** Each part uploads as `application/octet-stream` at an
   explicit offset, in chunks no larger than `maxRequestBytes`. The server
   hands the untouched stream to the spool, which writes it through to disk
   with a running byte cap. Re-sending from an earlier offset is how an
   interrupted upload resumes; a completed part settles to the same digest, so
   a duplicated request is not a duplicated corpus.
3. **Expansion.** The spooled bytes are read back a window at a time. An
   archive is walked through one random-access reader that understands ZIP64,
   streams each member through `inflateRaw`, and charges every budget as bytes
   appear rather than trusting the index. Each member is classified while it
   streams — encoding, control density, line length, JSON Lines validity, and
   the share-safe privacy gate — and staged at its content address only once it
   is accepted.
4. **Commit.** The persistence path reads exactly one staged member at a time,
   so peak resident bytes are one member rather than one corpus. The session's
   spool is removed on success.

Cancellation is a first-class outcome: it is recorded as intent the moment it
is asked for, polled between members and between windows, and everything that
follows it — including an I/O error from bytes being torn down mid-write — is
reported as the cancellation rather than as a storage failure. A cancelled or
failed session leaves no addressable staged member behind, and a boot-time
recovery sweep reclaims expired sessions and interrupted scratch files.

Refusals are named, not generic. `request_too_large`,
`expanded_budget_exceeded`, `file_count_exceeded`, `per_file_bytes_exceeded`,
`unsupported_encoding`, `unsafe_archive_path`, `archive_ratio_exceeded`,
`part_offset_conflict`, and the rest each carry the limit that was exceeded and
what was observed, because an operator who cannot tell "your archive is too
large" from "your archive has a `..` member" cannot act on either.
...
War Room corpus intake carries its own configurable bounds, defaulted
conservatively and clamped by published ceilings:

| Dimension                              |         Default |          Ceiling | Behavior                        |
| -------------------------------------- | --------------: | ---------------: | ------------------------------- |
| Intake HTTP request bytes              |           8 MiB |           64 MiB | `request_too_large`             |
| Compressed archive bytes               |          64 MiB |            2 GiB | `compressed_budget_exceeded`    |
| Total expanded bytes                   |         512 MiB |            4 GiB | `expanded_budget_exceeded`      |
| Files per intake                       |           4,096 |          100,000 | `file_count_exceeded`           |
| Bytes per file                         |          64 MiB |          128 MiB | `per_file_bytes_exceeded`       |
| Compression ratio                      |             256 |            4,096 | `archive_ratio_exceeded`        |
| Directory path depth                   |               8 |               64 | `path_depth_exceeded`           |
| Nested archive depth                   |               0 |                2 | `archive_depth_exceeded`        |
| Path length                            |    240 NFC chars |  1,024 NFC chars | `path_too_long`                 |
| Line bytes                             |           8 MiB |           64 MiB | `per_file_bytes_exceeded`       |
| Whole-text validation bytes            |          16 MiB |          128 MiB | `per_file_bytes_exceeded`       |
| Inline expansion wall clock            |            60 s |            600 s | `expansion_timeout`             |
| Streamed expansion wall clock          |           600 s |          3,600 s | `expansion_timeout`             |
| Accepted text encodings                | utf-8, us-ascii, utf-8-lossy | plus utf-16le/be | `unsupported_encoding` |

The request ceiling is the one that must stay executable: it is validated at
module load and at every override against `String::kMaxLength`, so an
advertised JSON body limit is always one the runtime can materialize. Peak
resident bytes for an intake are `maxRequestBytes + maxLineBytes +
2 * maxStructuredParseBytes + 4 * expansionWindow` and do not grow with the
corpus — a 512 MiB corpus and a 96 MiB corpus hold the same ceiling.
