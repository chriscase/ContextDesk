...
- **Import responsiveness (#824):** ingest reports measured wall-clock phases (discover/read, parse/frame, template analysis, persist/index, optional embedding, publication) via `ProcessProgress` (`elapsed_ms` / `phase_elapsed_ms`) and `IngestPhaseTimings` on the report. Drain mining is single-pass during parse/frame, so the short Template/Redact bookend phases may report near-zero ms while parse/frame carries the bulk of templating work—labels stay honest to the emitted phase enum. Desktop host runs ingest on `spawn_blocking`; the UI only renders progress events. Publication is atomic: cancel/failure before publish leaves no library corpus. Optional embedding may be deferred with an explicit completion note that keyword/structured first use is ready. Scale proofs (25k/100k/250k) record one-machine timings for same-machine regression budgets—not a universal SLA.
