...
| Triage Policy V2 and SDK parity | How can one or several qualified models contribute without making multi-model setup mandatory or creating separate CLI, GUI, and SDK truth? | **Partial**: the pure compiler and additive request/event/result contracts now exist; Standard remains the default. A default-off trusted-host subset adapter can now bind fully admitted observation/causal/contradiction/gap roles to the existing production linked contribution runtime with exact ModelRef, deadlines, cancellation, validation, reconciliation, rendering, and typed pre-provider refusals. Timeline, finalizer, reviewer, optional-dropout event projection, V2 terminal events, host resolver, and CLI/Tauri selection remain unwired | [Triage Policy V2](TRIAGE_POLICY_V2.md) · [core proof](../testing/TRIAGE_POLICY_CORE_V2.md) · [production subset proof](../testing/TRIAGE_POLICY_V2_PRODUCTION_ADAPTER_V1.md) · [#872](https://github.com/chriscase/ContextDesk/issues/872) |
