...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records, file-backed store, manual import, honest same-task/same-snapshot reports, and a hermetic public-SDK mock adapter (`compile`/`preflight`/`execute`/`cancel`/`replay`/`evaluate` over published schema ids, visibility-fail-closed packets, typed complete/failed/partial/timeout/cancel runs) in `cd-triage-bench`; live provider execution, object storage, web collaboration, and synthesis remain residual (#879 live path / #883) | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) |
