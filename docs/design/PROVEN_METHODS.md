...
| War Room investigation lifecycle | How does an investigation leave the working list without leaving the record, and come back to where it actually was? | **Local integration**: archiving is a confirmed, explained act separate from the ordinary status control; legal hold refuses an archive fail-closed before any write and never refuses a restore; restore reads the recorded `case_status` timeline and returns the investigation to the status it held, falling back to `open` and never to `resolved`; a read-only lifecycle route answers before the click so a refusal is stated rather than discovered; archived investigations leave the inventory with the withheld count reported beside the control that reveals them; the inventory predicate covers the situation text the card displays, involved-entity labels, and the recorded occurrence as literal text. Deletion does not exist and is answered rather than offered. Software/version/build tags, activity-filter UI, private notes, and `retentionClass` enforcement are named non-claims | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#14-shipped--partial--planned-matrix) · [Lifecycle gap report v1](WAR_ROOM_INVESTIGATION_LIFECYCLE_GAPS_V1.md) |
| UI strategy presentation | How can teams try a faster investigation workflow without creating a second authority over data or permissions? | **Shipped**: a pure, fail-closed registry resolves the War Room reference and one Investigation First adapter; both use the same protected case/evidence/lifecycle authority, preserve canonical URLs and record identities, and keep specialist log exploration in the reference workspace. **Active integration**: Runtime V1 moves new strategies behind one typed gateway and mounted, race-safe controller boundary, with Investigation First as the first required consumer and a non-growing legacy allowlist for War Room. Server-governed strategy policy, trash, and later strategies remain planned | [Investigation First strategy](../development/INVESTIGATION_FIRST_STRATEGY.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) |
