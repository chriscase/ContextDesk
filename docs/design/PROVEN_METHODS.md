...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records, file-backed store, manual import for human/web/other-product runs with byte-exact raw capture, immutable fairness, unknown-preserving provenance, duplicate/near-duplicate outcomes, and honest same-task/same-snapshot reports in `cd-triage-bench`; the validated public-SDK adapter and deterministic offline mock are promoted on `integrate/rc`; live ContextDesk execution, a batch runner, object storage, web collaboration, and synthesis remain #879/#883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) |
