...
| Activity transparency | How can a person inspect model, host, connector, permission, and governed-write work without turning activity into evidence? | **Partial**: one causal provider/host timeline, metadata-only default retention, opaque evidence ids, actual permission decisions, an explicit redacted/bounded process-local developer stream, and a nested human CLI Activity/trace hierarchy (ASCII-safe, TTY-aware) ship; import/reanalysis detail, connector-internal retries, and durable chat hydration remain residual | [Activity Inspector](ACTIVITY_INSPECTOR.md) |
