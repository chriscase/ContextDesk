...
| Investigation coordination / Operations Queue | Who is currently coordinating a visible investigation without inventing priority, SLA, presence, or automatic assignment? | **Local integration**: `InvestigationCoordinationV1` defines one nullable coordinator, monotonic revision, action/target-bound claim/release/assign envelopes, bounded refusals, and durable idempotency rules. Capability v2 protects privileged server actions. The server provides concealed coordination routes and a joined, server-ordered queue query. Runtime V1 exposes that query read-only, and the first-class `/operations` shell area renders only recorded title, status, and coordinator with server scope counts, cursor continuation, and query-only saved views. Investigation First already applies `claim_self` / `release_self` / `assign_participant` / `release_participant` on the mounted active case through `POST /api/cases/:id/coordination`. Operations rows remain read-only. **Accepted design**: the next named-row self-action seam reuses `claim_self` / `release_self` on that same case route (`investigation:write`); it does not add a queue write, operations endpoint, GET-per-row coordination, active-case retargeting, or privileged participant assignment/release. Packet order: contracts/docs → server proof → Runtime named-row command/controller → Operations UI/e2e. | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#operations-queue-coordination-foundation-local-server-integration) · [Named-row self-action](proven-methods/INVESTIGATION_LOOP.md#named-row-self-action-seam-accepted-design) · [Identity and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md#63-investigation-coordination-capability) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) |
