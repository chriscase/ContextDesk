...
| **Local integration** | Implemented or verified on an unmerged integration branch of this product only; not a separate local server | Branch/commit evidence; never described as available on `main` |
...
planned method to shipped. **Local integration** names unmerged work in this
same product; it is not a separate local server.
...
| Log evidence pipeline          | How do raw records become honest, searchable, scalable evidence?                       | **Partial**: inventory-bound reviewed import, bounded logical-record framing, deterministic grammar fingerprints, explainable noise review, and a host-authoritative exact-template lens ship; custom format profiles, full timestamp policy, broader suppression lifecycle, and cross-corpus baselines remain #751/#670/#671/#690; **Partial** War Room path on `main` (PR #1090) keeps a durable case-bound corpus with per-source preview/apply/clear/undo, revision and preview-fingerprint provenance, DST gap/fold reporting, and retained order-only evidence — no zone is inferred and bare abbreviations are refused | [Log evidence pipeline](proven-methods/LOG_EVIDENCE_PIPELINE.md) · [War Room log time review v1](../benchmarks/WAR_ROOM_LOG_TIME_REVIEW_V1.md) |
...
| Investigation loop             | How does an engineer preserve, revisit, and act on discoveries without losing control? | **Partial**: manual evidence/findings/notes, view recipes, the SoftWrite proposal review queue (findings + report sections), and the versioned accepted-state report projection with deterministic Markdown export have production anchors; **local integration** adds investigation-scoped War Room file/ZIP/directory intake that lands ordinary freeze/triage evidence, plus a fail-closed activity/resource-locator projection consumed by Overview as human-readable, copyable deep links; **local integration** also binds ordinary evidence/import writes to the staged content-addressed case transaction, compares held `expectedHash` before metadata/blob commit, freezes snapshots and hypothesis/tombstone revisions atomically, persists every successful SQLite CaseStore mutator, projects accepted board decisions only from the selected snapshot fingerprint, replays identical authorized contribution retries, fails closed on stale contribution revisions, records honest same-snapshot identity, and refuses expired or superseded triage-lease updates plus recovered queued execution that re-resolves current requester authority (profile/status/provenance, roles/grants, case access, `run:strategies`, `evidence:private:read`) before lease claim; **local integration** also records, once per time change, whether each existing snapshot now merely *reads* differently (`revised`, bytes unchanged) or whether a run's conclusions are superseded (`invalidated`), and says `unknown_basis` for work frozen before the case had a log corpus rather than implying it is still current; proposal ranking/walkthroughs, feed pagination/filter controls, and the fuller #532 report workflow remain residual | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md) · [War Room activity locators](../benchmarks/WAR_ROOM_ACTIVITY_RESOURCE_LOCATOR_V1.md) |
| Investigation loop             | How does an engineer preserve, revisit, and act on discoveries without losing control? | **Partial**: manual evidence/findings/notes, view recipes, the SoftWrite proposal review queue (findings + report sections), and the versioned accepted-state report projection with deterministic Markdown export have production anchors; **local integration** adds investigation-scoped War Room file/ZIP/directory intake that lands ordinary freeze/triage evidence and rejects ZIP names with invalid encodings, a missing UTF-8 language bit (`0x0800`), or a confused Info-ZIP Unicode Path extra (`0x7075`), plus a fail-closed activity/resource-locator projection consumed by Overview as human-readable, copyable deep links; **local integration** also binds ordinary evidence/import writes to the staged content-addressed case transaction, compares held `expectedHash` before metadata/blob commit, freezes snapshots and hypothesis/tombstone revisions atomically, persists every successful SQLite CaseStore mutator, projects accepted board decisions only from the selected snapshot fingerprint, replays identical authorized contribution retries, fails closed on stale contribution revisions, records honest same-snapshot identity, and refuses expired or superseded triage-lease updates plus recovered queued execution that re-resolves current requester authority (profile/status/provenance, roles/grants, case access, `run:strategies`, `evidence:private:read`) before lease claim; concurrent corpus digest classification, unpaired `file_server_ref` rollback, kind-strict locator resolve after portable identity remapping (including remapped decision ids, remapped gold snapshot ids from `experiment_gold_promoted`, remapped helpfulness observation ids from `experiment_helpfulness_recorded`, remapped `${experimentId}:${traceId}` interaction-trace locators from `experiment_trace_imported`, remapped experiment ids from `experiment_imported`, `${jobId}:${candidateId}` workstream attempts, and intake-batch ids from `corpus_intake_committed`), and pending-write journal recovery of unreferenced promote-before-COMMIT CAS residue, plus triage create/cancel/claim+started/candidate persist/finish and stale-lease recovery joining the case atomic boundary, running progress `update` preserving the later of store vs payload `leaseExpiresAt` so a concurrent heartbeat cannot be shortened by a stale candidate persist, and SQLite-wrapped experiment/run `capture`/`restore` awaited so timeline failure cannot leave a durable satellite row after reopen, plus human trace annotations persisting unique `(experimentId, candidateId, sequence)` inside the experiment lock without read-time renumbering, concurrent first gold promotions locking the experiment so identical fingerprints stay idempotent and a forked first gold is `stale_gold` rather than PostgreSQL `23505`, concurrent first interaction traces locking the experiment so identical fingerprints stay idempotent and a forked first trace is `trace_conflict` rather than PostgreSQL `23505`, with key-order-canonical `traceFingerprint` so PostgreSQL jsonb round-trips match memory, are covered in the investigation-loop chapter; proposal ranking/walkthroughs, feed pagination/filter controls, and the fuller #532 report workflow remain residual | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md) · [War Room activity locators](../benchmarks/WAR_ROOM_ACTIVITY_RESOURCE_LOCATOR_V1.md) |
...
| Composable investigation strategy connector | How can built-in triage, gateway lanes, manual imports, and future third-party methods share one snapshot-bound, append-only, comparison-ready contract? | **Accepted design**: versioned connector capability, run request/result/history, provider-free dry-run planner, and a no-winner comparison projection live in `@cd-collab/contracts`. No execution, credentials, networking, UI, or persistence ship in this slice | [Investigation strategy connector v1](../benchmarks/INVESTIGATION_STRATEGY_CONNECTOR_V1.md) |
| Bundled Help truth and delivery | How does shipped in-app documentation stay checkably true instead of silently rotting? | **Partial**: structural validation, bidirectional link/anchor resolution, whole-asset-set safety and theme-independent legibility, packaging assertion, and offline delivery ship; **local integration** adds a War Room cross-surface drift gate for Markdown image references, structured Help alt/captions, true PNG bytes, synchronized public/docs assets, complete static inlining, and canonical anchor identity, plus an accessible image lightbox; rendered visual quality remains human review | [Bundled Help truth and delivery](proven-methods/HELP_TRUTH_AND_DELIVERY.md) |
...
| War Room activity and deep links | How can an engineer read recent investigation activity and open the exact resource without treating a locator as an authorization token or inventing human labels? | **Local integration**: versioned `cdl.v1` locators, a deterministic timeline projection (not a second feed) that collapses rows describing one recorded action written under more than one event kind while keeping genuinely repeated work, membership/privacy fail-closed resolution, cursor pagination over that same collapsed list, memory/PostgreSQL store paging, an Overview consumer whose copyable pathnames land on shipped Discussion / workstream / run records, comparison lanes addressable as first-class records so a review-queue entry opens the lane's own row, and a presentation boundary that rejects shortened hashes as labels; restored history stays in the feed with its provenance stated but never counts as open work; Overview additionally groups repeats of one recorded activity before its window is taken — same investigation, kind, wording, target, actor, and provenance — and states the repeat count rather than dropping it, so two people's identical work stays two rows; and an arrival announcement never claims to have opened a record the surface does not show, reporting the surface while the record is still being looked for and saying so once it has not appeared; imported-run locators still address the evidence board rather than Capture, which needs an `imported_run` resource kind, and UI filtering/pagination and unbounded history remain residual | [War Room activity and resource locator v1](../benchmarks/WAR_ROOM_ACTIVITY_RESOURCE_LOCATOR_V1.md) |
| War Room activity and deep links | How can an engineer read recent investigation activity and open the exact resource without treating a locator as an authorization token or inventing human labels? | **Local integration**: versioned `cdl.v1` locators, a deterministic timeline projection (not a second feed), membership/privacy fail-closed resolution that also refuses kind-confused resource ids (including an intake-batch id presented as `evidence_item`), cursor pagination, memory/PostgreSQL store paging, and an Overview consumer whose copyable pathnames land on shipped Discussion / workstream / run / Capture intake-batch records; UI filtering/pagination and unbounded history remain residual | [War Room activity and resource locator v1](../benchmarks/WAR_ROOM_ACTIVITY_RESOURCE_LOCATOR_V1.md) |
| Identity, profile, capability, and administration | How does a collab installation give every authenticated person a stable, provenance-aware profile and enforce fine-grained capabilities, without ever letting a historical/imported actor gain authority? | **Partial**: canonical profile contract, capability model v3 with 12 fine-grained capabilities and additive local grants (`investigation:coordinate` is enforced by the action-specific coordination route; **local integration** enforces `catalog:write` on strict source-catalog CAS before parse/store access), Memory+PostgreSQL stores with CAS and fail-closed identity-collision resolution, domain-wide `authorizeSession` enforcement (including suspension/historical fail-closed and local grants), **local integration** of cookie-free `authorizeRecoveryRequester` for queued triage restart recovery so stale admin/private authority is never inherited, admin search/effective/suspend/grant routes, self-service profile API **and** `/profile` UI, and a first-class `/admin/people` shell location (with `/administration` as the roles alias) gated by `admin:users` ship; live LDAP attribute sync and automatic directory-removal detection remain residual (named non-claims, not gaps); CSRF retrofit of older mutation routes remains residual | [Identity, profile, capability, and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md) |
| Investigation coordination / Operations Queue | Who is currently coordinating a visible investigation without inventing priority, SLA, presence, or automatic assignment? | **Partial**: `InvestigationCoordinationV1` defines one nullable coordinator, monotonic revision, action/target-bound claim/release/assign envelopes, bounded refusals, and durable idempotency rules. Capability v2 protects privileged server actions. The server provides concealed coordination routes and a joined, server-ordered queue query. Runtime V1 exposes that query read-only (it is not a write path), and the first-class `/operations` shell area renders recorded title, status, and coordinator with server scope counts, cursor continuation, and query-only saved views. Investigation First already applies `claim_self` / `release_self` / `assign_participant` / `release_participant` on the mounted active case through `POST /api/cases/:id/coordination`. Named-row `claim_self` / `release_self` controls are a separate write seam from that read-only queue query: the merged Operations Queue self-coordination slice on `main` uses the public Runtime command `applyNamedCoordinationSelf` on that same case route (`investigation:write`). PR #1152 is a qualification-only e2e lane; hosted acceptance is not claimed until that lane is accepted or closed. There is no queue-side writer, operations write endpoint, GET-per-row coordination, or active-case retargeting. **Accepted design**: the next named-row privileged participant assign/release seam after this docs freeze reuses existing `assign_participant` / `release_participant` on that same case route (`investigation:coordinate`); it does not add a queue write, operations endpoint, GET-per-row coordination, active-case retargeting, people-search API, priority, SLA, ranking, presence, or automatic membership. This docs change ships no privileged queue controls. Packet order: contracts/docs → server proof → Runtime named-row privileged command/controller → Operations UI/e2e. This index does not authorize code until these docs are merged and a separate bounded implementation packet is issued. | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#operations-queue-coordination-foundation-partial-existing-server-integration) · [Named-row self-action](proven-methods/INVESTIGATION_LOOP.md#named-row-self-action-seam-partial-merged-on-main-qualification-pending) · [Named-row privileged assign and release](proven-methods/INVESTIGATION_LOOP.md#named-row-privileged-participant-assign-and-release-seam-accepted-design) · [Identity and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md#63-investigation-coordination-capability) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) |
| External-run human judgment | How can a presentation record a bounded human assessment of one imported run without turning it into a correctness verdict or changing legacy corroboration? | **Local integration**: the public investigation Runtime adds an explicit run query, frozen descriptive judgment resource, action-specific `investigation:write` projection, and one caller-triggered command. The gateway is the only browser route owner; 401/403 remain global auth loss, run 404 stays local, versioned conflict/refusal/limit failures are bounded, and an uncertain 503 can be retried only with its exact frozen request and idempotency identity. Local focused qualification passes 240 tests; serialized web passes 101 files / 1,608 tests and contracts pass 67 files / 935 tests. No strategy UI, browser/AT, hosted CI, or live-server claim is made, and legacy corroboration remains unchanged. | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#2-status-and-evidence) |
| War Room investigation lifecycle | How does an investigation leave the working list without leaving the record, and come back to where it actually was? | **Shipped**: archiving is a confirmed, explained act separate from the ordinary status control; the action-specific lifecycle route locks and reloads the case, status history, and legal hold, compares the parsed preview state, re-evaluates the action, and writes only the server-derived target in the same case/timeline/audit transaction. Changed state returns a versioned conflict with the current lifecycle and no action writes; bounded refusals preserve investigation identity and action. Restore returns to the most recent recorded working status, falling back to `open` and never to `resolved`; legal hold refuses archive and never refuses restore. Archived investigations leave the inventory with the withheld count reported beside the control that reveals them. Deletion does not exist and is answered rather than offered. Software/version/build tags, activity-filter UI, private notes, and `retentionClass` enforcement are named non-claims | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#14-shipped--partial--planned-matrix) · [Lifecycle gap report v1](WAR_ROOM_INVESTIGATION_LIFECYCLE_GAPS_V1.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md#lifecycle-command-integrity) |
| UI strategy presentation | How can teams try a faster investigation workflow without creating a second authority over data or permissions? | **Shipped**: a pure, fail-closed registry resolves War Room, Investigation First, and Keystone while preserving canonical URLs, record identities, and shared authority. Runtime V1 places investigation behavior behind one browser-safe typed gateway and mounted, race-safe controller boundary, provides an atomic server-derived lifecycle command, and holds remaining War Room direct calls to a non-growing legacy allowlist. **Local integration** adds UI Strategy Governance V1 plus Beacon/Rapid Intake: audited revision-checked instance/role rollout, an internal-identity-keyed personal preference, War Room-only malformed-policy recovery, a correctly split `admin:system_config` console, and an append-first presentation using only public create/upload/contribution/Situation commands. Policy/preference/audit writes are atomic; Beacon append, Situation promotion, and cited-hypothesis promotion remain separate caller actions. Server data/access/lifecycle/storage authority does not move. Trash and raw LDAP-group strategy rules remain planned | [Strategy governance and Beacon](../development/UI_STRATEGY_GOVERNANCE_AND_BEACON.md) · [Investigation First strategy](../development/INVESTIGATION_FIRST_STRATEGY.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) · [UI-strategy bakeoff v2](UI_STRATEGY_BAKEOFF_CONCEPTS_V2.md) |
| Investigation software impact | How can a triage engineer record which named product, version, build, component, or environment identities are observed, suspected, confirmed, or ruled out without inventing build order? | **Local integration**: investigation-scoped impact records, recording-order lists, case-insensitive identity keys that are not version comparators, visible-case suggestion values, and a Situation panel with accessible combo-boxes; PostgreSQL persistence, portable-archive inclusion, activity-kind projection, investigation tags/facets, and Help copy remain residual | [War Room software impact v1](../benchmarks/WAR_ROOM_SOFTWARE_IMPACT_V1.md) · [War Room next-release review](WAR_ROOM_NEXT_RELEASE.md) |
