...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records and file-backed store (#877 / #890/#891); manual import treats human, web-only, and other-product results as first-class `TriageRun`s with byte-exact raw capture, mandatory immutable fairness, per-field unknown provenance, and explicit duplicate / near-duplicate outcomes (#878); honest same-task/same-snapshot reports remain a sketch; ContextDesk SDK adapter, object storage, web collaboration, and synthesis remain #879/#883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) |
