...
| Investigation coordination / Operations Queue foundation | Who is currently coordinating a visible investigation without inventing priority, SLA, presence, or automatic assignment? | **Local server integration**: `InvestigationCoordinationV1` defines one nullable coordinator, monotonic revision, action/target-bound claim/release/assign envelopes, bounded refusals, a pure preloaded-subject evaluator, and durable idempotency rules. Capability v2 protects privileged changes. The server provides concealed GET/action POST routes, case-row-serialized projection plus insert-only successful replay, and atomic audit/timeline activity. Queue collection scope and UI remain outside this slice | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#operations-queue-coordination-foundation-local-server-integration) · [Identity and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md#63-investigation-coordination-capability) |
