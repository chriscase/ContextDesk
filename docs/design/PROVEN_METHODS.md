...
| Triage Policy V2 and SDK parity | How can one or several qualified models contribute without making multi-model setup mandatory or creating separate CLI, GUI, and SDK truth? | **Partial**: the additive request/event/result/replay contracts live in the leaf `cd-triage-sdk` crate (re-exported as `cd_core::triage_sdk` / `cd_core::model_ref`); the pure compiler and trusted host resolver remain in `cd-core` / `cd-workflow`. Standard remains the default and caller-authored preflight remains simulation-only. **Local integration** connects Standard/Saved/Inline execution to one `WorkflowTriageEngineV1` behind the provider-free `cd-triage-runtime` façade: canonical request identity, preflight-before-execute, an immutable prepared packet, exact gateway-scoped model binding, exact cancellation, Standard's six-event graph, request-bound provisional streaming, authoritative replay return, cross-bound terminal validation, and distinct completed/partial/failed/timed-out/cancelled projections. The #879 source-neutral adapter consumes the same canonical identity and rejects replay packet/model/reconciliation/validation drift before recording. The live same-snapshot bench bridge remains open | [Triage Policy V2](TRIAGE_POLICY_V2.md) · [runtime/bench bridge](TRIAGE_RUNTIME_BENCH_BRIDGE_V1.md) · [core proof](../testing/TRIAGE_POLICY_CORE_V2.md) · [production subset proof](../testing/TRIAGE_POLICY_V2_PRODUCTION_ADAPTER_V1.md) · [#872](https://github.com/chriscase/ContextDesk/issues/872) |
