...
| UI strategy presentation | How can teams try a faster investigation workflow without creating a second authority over data or permissions? | **Shipped**: a pure, fail-closed registry resolves the War Room reference and one Investigation First adapter; both preserve canonical URLs and record identities and keep specialist log exploration in the reference workspace. Runtime V1 places investigation behavior behind one browser-safe typed gateway and mounted, race-safe controller boundary, provides an atomic server-derived lifecycle command, makes Investigation First its first presentation-only consumer, and holds remaining War Room direct calls to a non-growing legacy allowlist. **Local integration** adds optional Runtime V1.1 contribution-create and Situation-update seams with identity/authority/case/capability/unmount fencing, server-arbitrated Situation conflicts, and server-authoritative same-investigation hypothesis-link validation before any revision, idempotency, timeline, or audit write. The Runtime V1 public surface is frozen; additions remain additive and versioned. Server-governed strategy policy, trash, and later strategies remain planned | [Investigation First strategy](../development/INVESTIGATION_FIRST_STRATEGY.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) · [UI-strategy bakeoff v2](UI_STRATEGY_BAKEOFF_CONCEPTS_V2.md) |
