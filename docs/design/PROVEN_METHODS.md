...
| UI strategy presentation | How can teams try a faster investigation workflow without creating a second authority over data or permissions? | **Local integration**: a pure, fail-closed registry resolves the War Room reference and one Investigation First adapter; both consume the protected case/evidence/lifecycle APIs, preserve canonical URLs and record identities, and keep specialist log exploration in the reference workspace. Server-governed strategy policy, persisted preferences, trash, and later strategies remain planned | [Investigation First strategy](../development/INVESTIGATION_FIRST_STRATEGY.md) |
