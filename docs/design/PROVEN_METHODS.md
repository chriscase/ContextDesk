...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records and file-backed store (#877 / #890/#891); manual import treats human, web-only, and other-product results as first-class `TriageRun`s (#878); rubric v1 plus file/CLI expert adjudication (#880); deterministic report-only comparison (JSON/JSONL/markdown, same-task/same-snapshot groups, incomparable pairs, N vs N−1 drill-down, share_safe privacy scan) over imported records (#881 report-only); SDK batch runner residual until #879; object storage, web collaboration, and synthesis remain #883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) · [Rubric v1](../../crates/cd-triage-bench/docs/RUBRIC_V1.md) |
