...
| Activity transparency | How can a person inspect model, host, connector, permission, and governed-write work without turning activity into evidence? | **Partial**: one causal provider/host timeline, metadata-only default retention, truthful reproducibility classes, durable Summary journal, and actual permission decisions ship; connector-internal detail and CLI rendering remain residual | [Activity Inspector](ACTIVITY_INSPECTOR.md) |
