...
| Identity, profile, capability, and administration | How does a collab installation give every authenticated person a stable, provenance-aware profile and enforce fine-grained capabilities, without ever letting a historical/imported actor gain authority? | **Partial**: canonical profile contract, capability model v2 with 11 fine-grained capabilities and additive local grants (the new `investigation:coordinate` capability is contract-reserved, not yet enforced by a route), Memory+PostgreSQL stores with CAS and fail-closed identity-collision resolution, domain-wide `authorizeSession` enforcement (including suspension/historical fail-closed and local grants), **local integration** of cookie-free `authorizeRecoveryRequester` for queued triage restart recovery so stale admin/private authority is never inherited, admin search/effective/suspend/grant routes, self-service profile API **and** `/profile` UI, and a first-class `/admin/people` shell location (with `/administration` as the roles alias) gated by `admin:users` ship; live LDAP attribute sync and automatic directory-removal detection remain residual (named non-claims, not gaps); CSRF retrofit of older mutation routes remains residual | [Identity, profile, capability, and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md) |
| Investigation coordination / Operations Queue foundation | Who is currently coordinating a visible investigation without inventing priority, SLA, presence, or automatic assignment? | **Accepted contract / reserved capability**: `InvestigationCoordinationV1` defines one nullable coordinator, monotonic revision, action/target-bound claim/release/assign envelopes, bounded refusals, a pure preloaded-subject evaluator, and durable idempotency rules. Capability v2 reserves `investigation:coordinate` for case leads/admins and additive grants; its closed session enum and database constraint deploy together. No server route/store, queue query scope, audit/timeline writer, or UI ships in this slice | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#operations-queue-coordination-foundation-accepted-contract) · [Identity and administration](proven-methods/IDENTITY_AND_ADMINISTRATION.md#63-reserved-investigation-coordination-capability) |
