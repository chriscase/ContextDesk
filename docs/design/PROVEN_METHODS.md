...
| Triage Policy V2 and SDK parity | How can one or several qualified models contribute without making multi-model setup mandatory or creating separate CLI, GUI, and SDK truth? | **Partial**: the pure compiler and additive request/event/result contracts now exist; Standard remains the default. A trusted host resolver and CLI `triage run --request ...` bind admitted roles to the production linked contribution runtime using host-owned exact role qualification, ModelRef, deadlines, cancellation, validation, reconciliation, rendering, typed result, and replay. Caller-authored preflight remains provider-free simulation input and cannot authorize runtime credentials or egress. Visible optional-dropout projection, incremental surface streaming, and broader surface parity remain open | [Triage Policy V2](TRIAGE_POLICY_V2.md) · [core proof](../testing/TRIAGE_POLICY_CORE_V2.md) · [production subset proof](../testing/TRIAGE_POLICY_V2_PRODUCTION_ADAPTER_V1.md) · [#872](https://github.com/chriscase/ContextDesk/issues/872) |
...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records and file-backed store (#877 / #890/#891); manual import treats human, web-only, and other-product results as first-class `TriageRun`s (#878); rubric v1 plus packet-bound, chronology-checked file/CLI expert adjudication (#880); deterministic report-only comparison (JSON/JSONL/markdown, same-task/same-snapshot groups, incomparable pairs, source/build-partitioned N vs N−1 drill-down, partial coverage, share_safe privacy scan) over imported records (#881 report-only); SDK batch runner residual until #879; object storage, web collaboration, and synthesis remain #883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) · [Rubric v1](../../crates/cd-triage-bench/docs/RUBRIC_V1.md) |
