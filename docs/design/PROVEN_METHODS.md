...
| Provider/model compatibility qualification | Which exact gateway model has demonstrated the contracts required for one role without confusing compatibility with answer quality? | **Local integration**: shared GUI/CLI `Discover → Verify → Choose`, secret-free exact-identity evidence, subset-first large-catalog selection, explicit synthetic probes, and drift-aware re-verification are implemented; ordinary-chat, attachment, multimodal, and answer-quality evidence remain separate residuals | [Demo and model evaluation lab](proven-methods/DEMO_EVALUATION_LAB.md#64-run-ordinary-chat-and-capability-controls) |
...
| Triage Policy V2 and SDK parity | How can one or several qualified models contribute without making multi-model setup mandatory or creating separate CLI, GUI, and SDK truth? | **Partial**: the additive request/event/result/replay contracts now live in the leaf `cd-triage-sdk` crate (re-exported as `cd_core::triage_sdk` / `cd_core::model_ref`); the pure compiler and trusted host resolver remain in `cd-core` / `cd-workflow`. Standard remains the default. CLI `triage run --request ...` still binds admitted roles to the production linked contribution runtime. Caller-authored preflight remains provider-free simulation input and cannot authorize runtime credentials or egress. The #879 bench adapter now consumes those contracts from a sibling crate without an engine dependency, but only through a deterministic mock: no live `triage()` / `triage_with_policy()` entry point exists. Visible optional-dropout projection, incremental surface streaming, live bench execution, and broader surface parity remain open | [Triage Policy V2](TRIAGE_POLICY_V2.md) · [core proof](../testing/TRIAGE_POLICY_CORE_V2.md) · [production subset proof](../testing/TRIAGE_POLICY_V2_PRODUCTION_ADAPTER_V1.md) · [#872](https://github.com/chriscase/ContextDesk/issues/872) |
...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records, file-backed store, manual import for human/web/other-product runs with byte-exact raw capture, immutable fairness, unknown-preserving provenance, duplicate/near-duplicate outcomes, rubric v1 plus packet-bound, chronology-checked blinded file/CLI adjudication with preserved disagreement and immutable scores under re-adjudication, and deterministic report-only comparison (JSON/JSONL/markdown, same-task/same-snapshot groups, incomparable pairs, source/build-partitioned N vs N−1 drill-down, partial coverage, and share-safe privacy scan) over imported records in `cd-triage-bench`; the validated public-SDK adapter and deterministic offline mock are promoted on `integrate/rc`; live ContextDesk execution, a batch runner, object storage, web collaboration, and synthesis remain #879/#883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) · [Rubric v1](../../crates/cd-triage-bench/docs/RUBRIC_V1.md) |
| Collaborative Experiment Lab qualification | Does the current collab case/experiment path still create a case, freeze evidence, compare hermetic lanes, import an external chat, adjudicate, export share-safe, and keep lineage without inventing live results? | **Local integration**: `npm run qualify` walks those seams on the current Experiment Lab and case-memory stores with in-process fake host lanes (no second jobs/lease table); live profiles skip with an explicit reason; PostgreSQL is covered when `COLLAB_TEST_ADMIN_URL` is set; hosted `collab-qualify.yml` runs memory + PostgreSQL qualify, doctor, and sanitized artifacts without LDAP/Vercel/model calls; collab remains isolated from `cd-workflow` | [Current-architecture qualification](../testing/COLLAB_RELEASE_QUALIFICATION_CURRENT_ARCH_V1.md) · [Hosted CI](../testing/COLLAB_HOSTED_RELEASE_QUALIFICATION_V1.md) · [Experiment Lab adjudication](../benchmarks/EXPERIMENT_LAB_HUMAN_ADJUDICATION_V1.md) |
| Collaborative War Room operator readiness | Can an operator tell whether a demo or private collab install is configured safely without contacting providers? | **Local integration**: compile-first `npm run doctor` and `npm run config:init` inspect configuration shape (Node, artifacts, storage keys, evidence root, static dir, auth/LDAP TLS, cookies, optional host-bridge path, profile-catalog syntax, port); they never contact PostgreSQL, LDAP, Vercel, or model providers and never invent live results | [Operator readiness](../testing/COLLAB_OPERATOR_READINESS_V1.md) |
