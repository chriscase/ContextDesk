...
| War Room investigation lifecycle | How does an investigation leave the working list without leaving the record, and come back to where it actually was? | **Local integration**: archiving is a confirmed, explained act separate from the ordinary status control; the action-specific lifecycle route locks and reloads the case, status history, and legal hold, compares the parsed preview state, re-evaluates the action, and writes only the server-derived target in the same case/timeline/audit transaction. Changed state returns a versioned conflict with the current lifecycle and no action writes; bounded refusals preserve investigation identity and action. Restore returns to the most recent recorded working status, falling back to `open` and never to `resolved`; legal hold refuses archive and never refuses restore. Archived investigations leave the inventory with the withheld count reported beside the control that reveals them. Deletion does not exist and is answered rather than offered. Software/version/build tags, activity-filter UI, private notes, and `retentionClass` enforcement are named non-claims | [Investigation loop](proven-methods/INVESTIGATION_LOOP.md#14-shipped--partial--planned-matrix) · [Lifecycle gap report v1](WAR_ROOM_INVESTIGATION_LIFECYCLE_GAPS_V1.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md#lifecycle-command-integrity) |
| UI strategy presentation | How can teams try a faster investigation workflow without creating a second authority over data or permissions? | **Shipped**: a pure, fail-closed registry resolves the War Room reference and one Investigation First adapter; both preserve canonical URLs and record identities and keep specialist log exploration in the reference workspace. **Local integration candidate**: Runtime V1 places investigation behavior behind one browser-safe typed gateway and mounted, race-safe controller boundary, provides an atomic server-derived lifecycle command, makes Investigation First its first presentation-only consumer, and holds remaining War Room direct calls to a non-growing legacy allowlist. Exact-head external acceptance and the contract freeze remain open. Server-governed strategy policy, trash, and later strategies remain planned | [Investigation First strategy](../development/INVESTIGATION_FIRST_STRATEGY.md) · [Investigation Runtime V1](../development/INVESTIGATION_RUNTIME_V1.md) · [UI-strategy bakeoff v2](UI_STRATEGY_BAKEOFF_CONCEPTS_V2.md) |
