...
| Incident-triage evaluation bench | How can whole-strategy triage attempts on frozen historical incidents be recorded, adjudicated, and compared without a GUI and without inventing readiness? | **Local integration**: versioned case/snapshot/task/run/adjudication/score records, file-backed store, manual import, and honest same-task/same-snapshot reports in `cd-triage-bench`; a draft `cd-triage-bench-adapter` runs ContextDesk as one strategy over the public SDK contracts with a deterministic offline mock, owner-only recorded runs, and an explicit share-safe projection; live ContextDesk execution, a batch runner, object storage, web collaboration, and synthesis remain #879/#883 | [Triage bench v1](../benchmarks/TRIAGE_BENCH_V1.md) |
