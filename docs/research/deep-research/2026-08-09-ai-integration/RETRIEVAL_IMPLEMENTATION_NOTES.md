...
- Explicitly enabled retrieval roles now attach through the desktop and CLI's
  existing `ToolHost` seams. Linked-log search can use the configured
  embedding and reranker backends, while preserving trusted event identities and the
