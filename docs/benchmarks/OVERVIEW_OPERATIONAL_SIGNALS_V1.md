...
- The server does not load the complete case/participant inventory or pass an
  unbounded visible-id list. PostgreSQL counts and projections enforce
  membership inside their queries, and bounded downstream joins apply `LIMIT`.
  Memory and SQLite stores use a direct process-local case index for visibility,
  so Overview does not issue per-candidate case-store reads; SQLite read methods
  do not rewrite serialized state.
